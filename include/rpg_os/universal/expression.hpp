...
// AST formula evaluator (universal / dynamic mode).
//
...
// EvalContext that resolves identifiers (bare ids, `actor.*`, `target.*`,
// `env.*`, `event.*`, `action.*`).
//
...
/// Resolves a formula identifier to a value. Returns false when the path is
/// unknown (the evaluator then throws std::runtime_error).
class EvalContext {
public:
...
class Expression {
public:
  /// Default-constructs the empty expression (evaluates to 0.0).
  Expression() = default;
